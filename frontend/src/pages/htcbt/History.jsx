...
import { useHtcbtRuns, useHtcbtDeleteRun, fmtFull } from "@/lib/htcbt/api";
...
export default function HtcbtHistory() {
  const { data: runs = [], isLoading } = useHtcbtRuns();
  const del = useHtcbtDeleteRun();

  if (isLoading) {
    return <div className="flex justify-center py-24" data-testid="htcbt-history-loading"><Loader2 className="h-6 w-6 animate-spin text-amber-400" /></div>;
  }
  if (!runs.length) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center" data-testid="htcbt-history-empty">
  ...
  return (
    <div className="flex flex-col gap-3 animate-fade-up" data-testid="htcbt-history">
      {runs.map((r) => {
        const st = STATUS[r.status] || STATUS.stopped;
        return (
          <div key={r.id} data-testid={`htcbt-run-${r.id}`} className="rounded-xl border border-zinc-700 bg-zinc-900 p-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 font-mono text-[9px] font-bold tracking-widest ${st.cls}`}>
                {r.status === "done" ? <CheckCircle2 className="h-3 w-3" /> : r.status === "stopped" ? <Square className="h-3 w-3" /> : <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-amber-400" />}
                {st.label}
              </span>
            </div>
            ...
            <div className="mt-3 grid gap-1 font-mono text-[11px] text-zinc-400">
              ...
              <div>Finish : <span className="text-amber-400">{fmtFull(r.finish_at)}</span></div>
              {r.operator && <div>Operator: <span className="text-zinc-200">{r.operator}</span></div>}
            </div>
            <div className="mt-3 flex justify-end">
              <button
                data-testid={`htcbt-delete-run-${r.id}`}
